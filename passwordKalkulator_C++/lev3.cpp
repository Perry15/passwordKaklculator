...
lev3::lev3(QString p): lenMin(10), Password(p,p.length()){
...
lev3::~lev3(){}
QString lev3::getDescription() const{
    return description;
}
QString lev3::combination1(const startingData &d1, const startingData &d2){
    //LEV3 LUNGHEZZA MINIMA 10 LETTERE MINUSCOLE E MAIUSCOLE E NUMERI E CARATTERI SPECIALI
    ...
    r = qrand() % ((3 - 0) - 0) + 0;//indice casuale per estrarre 3 elementi da d1
    int x = qrand() % ((3 - 0) - r) + 1;
    ...
    //se non ci sono caratteri speciali
    if(ris.toStdString().find_first_of("€\\|&<>`~!£$%/()=?^'[]{}*+-#@°ç,._;:\"") == std::string::npos){
        //inserisco due caratteri speciali casuali in delle posizioni casuali
        QString charSpec="&!^$%/=?*+-#@";//13 caratteri speciali
        l = qrand() % ((13) -0) + 0;//tra 0 e 13 escluso
        r=qrand() % ((ris.length()) -0) + 0;//indice casuale
        ris.insert(r,charSpec.mid(l,1));//aggiungo primo carattere
        l = qrand() % ((13) -0) + 0;//tra 0 e 13 escluso
        r=qrand() % ((ris.length()) -0) + 0;//indice casuale
        ris.insert(r,charSpec.mid(l,1));//aggiungo secondo carattere
    }
...
QString lev3::combination2(const startingData &d1, const startingData &d2){
    //LEV3 LUNGHEZZA MINIMA 10 LETTERE MINUSCOLE E MAIUSCOLE E NUMERI E CARATTERI SPECIALI
    ...
    if(ris.toStdString().find_first_of("€\\|&<>`~!£$%/()=?^'[]{}*+-#@°ç,._;:\"") == std::string::npos){//se non ci sono caratteri speciali
        //inserisco due caratteri speciali casuali in delle posizioni casuali
        QString charSpec="&!^$%/=?*+-#@";//13 caratteri speciali
        l = qrand() % ((13) -0) + 0;//tra 0 e 13 escluso
        r=qrand() % ((ris.length()) -0) + 0;//indice casuale
...
QString lev3::combination3(const startingData &d1, const startingData &d2){
    //LEV3 LUNGHEZZA MINIMA 10 LETTERE MINUSCOLE E MAIUSCOLE E NUMERI E CARATTERI SPECIALI
